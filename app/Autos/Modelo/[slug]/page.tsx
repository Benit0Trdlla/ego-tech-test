...
export default async function Page({ params }: { params: { slug: string } }) {
  ...
  const cleanDescription = car.description.replace(/<[^>]*>/g, '');
        ...
        <section className="container mx-auto m-6 p-6">
          <div className="items-center justify-center grid grid-cols-1 md:grid-cols-2 grid-rows-1 gap-10 md:m-6 md:p-6">
            ...
            <div>
              ...
              <h3 className="text-6xl font-bold pb-5 text-balance">{car.title}</h3>
              <p className="text-[18px]/[21px] py-5">{cleanDescription}</p>
            </div>
          </div>
        </section>
